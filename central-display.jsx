...
const ROUND_RULES_CD = [
  { round:1, name:"The Hawk Returns",       color:"#00f5c4", icon:"🏛️", volMult:0.8, leaderHidden:false, briefing:"Fed hawkishness reprices risk." },
  { round:2, name:"Silicon Iron Curtain",   color:"#fbbf24", icon:"🛰️", volMult:1.1, leaderHidden:false, briefing:"Industrial policy and export controls hit the tape." },
  { round:3, name:"Oil Strait Crisis",      color:"#ef4444", icon:"🛢️", volMult:1.35, leaderHidden:false, briefing:"A shipping shock drives oil higher and liquidity lower." },
  { round:4, name:"Debt Ceiling Roulette",  color:"#f97316", icon:"🏦", volMult:1.25, leaderHidden:false, briefing:"Funding stress and Treasury brinkmanship rattle markets." },
  { round:5, name:"Election-Year America",  color:"#a78bfa", icon:"🗳️", volMult:1.15, leaderHidden:true,  briefing:"The leaderboard goes dark under policy uncertainty." },
  { round:6, name:"Credit Crack",           color:"#fb923c", icon:"⚡", volMult:1.7, leaderHidden:false, briefing:"Credit spreads blow out and recession fear deepens." },
  { round:7, name:"America Reprices",       color:"#e879f9", icon:"🦅", volMult:2.0, leaderHidden:false, briefing:"Everything reprices in the final stress round." },
];
...
function getDisplayInferenceClues(events = []) {
  ...
  return [
    "Translate the macro bulletin into financing pressure, demand shifts, and pricing power.",
    "Let the tape confirm your thesis instead of expecting the bulletin to name names.",
    "Second-order winners and losers often matter more than the obvious first move.",
  ];
}
...
// ─── DISRUPTION CARD ─────────────────────────────────────────────────────────
function DisruptionCard({ events, bufferLeft, phase = "buffer" }) {
  const [revealed, setRevealed] = useState(0);
  const eventCount = events?.length || 0;
  const eventsKey = (events || []).map(evt => [
  ...
  useEffect(()=>{ setRevealed(0); },[eventsKey]);
      ...
      <div style={{
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:900, fontSize:20,
            color:"#fef2f2", letterSpacing:"-0.01em" }}>
            {liveDisruption ? "MARKET DISRUPTION LIVE" : "MARKET DISRUPTION ALERT"}
          </div>
          <div style={{ fontSize:10, color:"#fca5a5", letterSpacing:"0.2em", marginTop:2 }}>
            {liveDisruption ? "REPRICING NOW ACROSS THE MARKET" : "TAKING EFFECT NEXT ROUND"}
          </div>
        </div>
        {!liveDisruption && bufferLeft!=null && (
        ...
        {macroInferencePack && leadEvent ? (
          <div style={{
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ fontSize:24 }}>{leadEvent.eventIcon || "🧭"}</div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:9, color:"#38bdf8", fontWeight:800, letterSpacing:"0.14em", marginBottom:4 }}>
                  CENTRAL INFERENCE BRIEF
                </div>
                <div style={{ fontSize:18, color:"#f8fafc", fontWeight:800, lineHeight:1.35 }}>
                  {leadEvent.eventName || leadEvent.headline || "Macro bulletin"}
                </div>
              </div>
              <div style={{ fontSize:9, color:"#475569", letterSpacing:"0.12em" }}>
                NO DIRECT TICKER HINTS
              </div>
            </div>

            <div style={{ background:"#020817", border:"1px solid #1e293b", borderRadius:10, padding:"12px 14px" }}>
              <div style={{ fontSize:8, color:"#fbbf24", fontWeight:800, letterSpacing:"0.14em", marginBottom:6 }}>
                INFERENCE BRIEF
              </div>
              <div style={{ fontSize:13, color:"#e2e8f0", lineHeight:1.7 }}>
                {leadEvent.storylineBlurb || leadEvent.eventSubheadline || leadEvent.detail || "Teams must infer the affected names from the macro bulletin and market reaction."}
              </div>
            </div>

            <div style={{ background:"#020817", border:"1px solid #1e293b", borderRadius:10, padding:"12px 14px" }}>
              <div style={{ fontSize:8, color:"#38bdf8", fontWeight:800, letterSpacing:"0.14em", marginBottom:8 }}>
                WHAT TEAMS SHOULD INFER
              </div>
              <div style={{ display:"grid", gap:7 }}>
                {inferenceClues.slice(0, 3).map((clue, index) => (
                  <div key={`${index}:${clue}`} style={{ display:"flex", gap:8, fontSize:12, color:"#94a3b8", lineHeight:1.6 }}>
            ...
            {leadEvent.macroQuestion && (
              <div style={{ background:"#020817", border:"1px solid #1e293b", borderRadius:10, padding:"12px 14px" }}>
                <div style={{ fontSize:8, color:"#fbbf24", fontWeight:800, letterSpacing:"0.14em", marginBottom:6 }}>
                  TEAM CHALLENGE
                </div>
                <div style={{ fontSize:12, color:"#e5e7eb", lineHeight:1.65 }}>
                  {leadEvent.macroQuestion}
                </div>
              </div>
            )}
...
function CentralDisruptionTakeover({ events, bufferLeft, newsGroups, roundNum, phase, currentTime }) {
  ...
  const fallbackFeed = macroFallback
    ? [macroFallback]
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize:34, animation:"blink 0.8s step-end infinite" }}>🚨</div>
        <div>
          <div style={{ fontFamily:"'Syne',sans-serif", fontSize:26, fontWeight:900, color:"#fee2e2", letterSpacing:"-0.02em" }}>
            {liveDisruption ? "LIVE DISRUPTION ON THE TAPE" : `ROUND ${roundNum} DISRUPTION BULLETIN`}
          </div>
          <div style={{ fontSize:11, color:"#fca5a5", letterSpacing:"0.16em", marginTop:4 }}>
            {liveDisruption ? "MARKET PRICES ARE REPRICING NOW" : "FULL-SCREEN BUFFER BRIEFING ON CENTRAL DISPLAY"}
          </div>
        </div>
        <div style={{ marginLeft:"auto", textAlign:"right" }}>
          <div style={{ fontSize:10, color:"#94a3b8", letterSpacing:"0.14em" }}>CENTRAL CLOCK</div>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:30, lineHeight:1, color:"#f8fafc" }}>{currentTime}</div>
        </div>
      </div>

      <div style={{ minHeight:0, display:"grid", gridTemplateColumns:"minmax(0,1.35fr) minmax(320px,0.85fr)", gap:18, padding:"18px 20px 16px" }}>
        <div style={{ minHeight:0 }}>
          <DisruptionCard events={events} bufferLeft={liveDisruption ? null : bufferLeft} phase={phase}/>
        </div>
        ...
        <div style={{
          ...
          <div style={{ padding:"14px 16px 12px", borderBottom:"1px solid #111827" }}>
            <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:900, fontSize:18, color:"#f8fafc" }}>
              ROUND {roundNum} NEWS DESK
            </div>
            <div style={{ fontSize:10, color:"#475569", letterSpacing:"0.14em", marginTop:4 }}>
              TEAM-FACING NEWS, STORY BEATS, AND DISRUPTION CONTEXT
            </div>
          </div>
          <div style={{ flex:1, overflowY:"auto", padding:"12px 14px" }}>
            {visibleFeed.length === 0 ? (
              <div style={{ height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:12 }}>
                <div style={{ fontSize:32, opacity:0.14 }}>📰</div>
                <div style={{ fontSize:11, color:"#334155", letterSpacing:"0.16em" }}>AWAITING NEWS FEED</div>
              </div>
            ) : visibleFeed.map((item, index) => {
              const stock = findStockMeta(item.ticker);
              const tone = item.sentiment === "bull"
                ? "#00f5c4"
                : item.sentiment === "bear"
                  ? "#ef4444"
                  : "#38bdf8";
              return (
                <div key={`${item.ticker}-${item.time || index}-${index}`} style={{
                  background:"#0a0f1e",
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:5, flexWrap:"wrap" }}>
                    {!item.hideTicker && (
                      <span style={{ color:stock.color, fontWeight:800, fontSize:12 }}>{item.ticker}</span>
                    )}
                    {item.tag && (
                    ...
                    <span style={{ marginLeft:"auto", fontSize:8, color:"#475569" }}>{item.time}</span>
                  </div>
                  <div style={{ fontSize:12, color:"#e2e8f0", lineHeight:1.5, fontWeight:700 }}>
                    {item.headline}
                  </div>
                  {item.detail && (
                  ...
                  {Array.isArray(item.playerClues) && item.playerClues.length > 0 && (
                    <div style={{ marginTop:8, display:"grid", gap:5 }}>
                      {item.playerClues.slice(0, 3).map((clue, clueIndex) => (
                  ...
                  {item.macroQuestion && (
                    <div style={{ marginTop:8, padding:"8px 10px", background:"#0f172a", border:"1px solid #1e293b", borderRadius:8 }}>
                      ...
                      <div style={{ fontSize:10, color:"#e5e7eb", lineHeight:1.55 }}>{item.macroQuestion}</div>
                    </div>
                  )}
                </div>
              );
      ...
      <div style={{ height:34, borderTop:"1px solid #111827", background:"#030712", display:"flex", alignItems:"center", padding:"0 20px", gap:14 }}>
        <div style={{ fontSize:9, color:"#475569", letterSpacing:"0.14em" }}>
          {liveDisruption ? "DISRUPTION MODE" : "BUFFER MODE"}
        </div>
        <div style={{ fontSize:9, color:"#1e293b" }}>·</div>
        <div style={{ fontSize:9, color:"#475569" }}>ROUND {roundNum}/{TOTAL_ROUNDS}</div>
        <div style={{ fontSize:9, color:"#1e293b" }}>·</div>
        <div style={{ fontSize:9, color:"#475569" }}>
          {events.length} MARKET EVENT{events.length === 1 ? "" : "S"}
        </div>
        {!liveDisruption && bufferLeft != null && (
          <>
            <div style={{ fontSize:9, color:"#1e293b" }}>·</div>
...
function CentralDisplay() {
  ...
  const ranked = (finalMode ? sortFinalEntries : sortLiveEntries)(leaderboard.map(e=>{
    ...
    return {...e, total, roi, score, holdVal, pnl, uniquePositions};
  }));

  const tickerTapeStocks = Object.keys(prices || {}).length
    ? Object.keys(prices).map(ticker => findStockMeta(ticker))
    : STOCKS;
  ...
  const takeoverRoundNum = showDisruptionTakeover
    ? (Number.isFinite(disruptions?.[0]?.storylineRound)
        ? disruptions[0].storylineRound
        : Math.min(roundNum + 1, TOTAL_ROUNDS))
    : roundNum;
      ...
      <div style={{
        ...
        <div style={{ width:420, padding:48, animation:"fadeup 0.5s ease", position:"relative", zIndex:1 }}>
          ...
          <div style={{ textAlign:"center", marginTop:20, fontSize:9, color:"#1e293b" }}>
            BULL PIT &copy; 2025 &middot; CENTRAL DISPLAY v4
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{
              ...
              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                ...
                <div style={{ textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:9, color:"#334155" }}>
                    ...
                    {rules.leaderHidden && " · BOARD HIDDEN"}
      ...
      {showDisruptionTakeover ? (
        <CentralDisruptionTakeover
          events={disruptions}
              ...
              <div style={{ flex:1, overflowY:"auto", padding:"12px 16px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, alignContent:"start" }}>
                {leaderHidden && !finalMode ? (
                  <div style={{ gridColumn:"1 / -1", display:"flex", flexDirection:"column", alignItems:"center",
                    ...
                    <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:20, color:"#a78bfa" }}>
                      LEADERBOARD HIDDEN
                    </div>
                    <div style={{ maxWidth:420, color:"#64748b", fontSize:13, lineHeight:1.7 }}>
                      This round is running under incomplete-information rules. Teams must trade the policy narrative, not each other.
                    </div>
                  </div>
                ) : ranked.length===0 ? (
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)",
                        gap:8, marginBottom:10 }}>
                          {[
                            ...
                            { label:"SECTORS",  value:`${entry.uniquePositions||0} active`,
                              color:"#fbbf24" },
                          ].map(ind=>(
            ...
            <div style={{ display:"flex", flexDirection:"column", overflow:"hidden" }}>
              <div style={{ padding:"14px 16px 12px", borderBottom:"1px solid #0f172a" }}>
                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:900,
                  fontSize:16, color:"#f1f5f9" }}>NEWSWIRE & LIVE TRADES</div>
                <div style={{ fontSize:9, color:"#334155", letterSpacing:"0.1em",
                  marginTop:3 }}>DISRUPTIONS, SILENT STORY BEATS, AND EXECUTION FLOW</div>
              </div>
              <div style={{ flex:1, display:"grid", gridTemplateRows:"minmax(0,1.1fr) minmax(0,0.9fr)", overflow:"hidden" }}>
                <div style={{ minHeight:0, display:"flex", flexDirection:"column", borderBottom:"1px solid #0f172a" }}>
                  <div style={{ padding:"10px 12px 8px", borderBottom:"1px solid #0f172a" }}>
                    <div style={{ fontSize:11, fontWeight:800, color:"#38bdf8", letterSpacing:"0.12em" }}>
                      TEAM NEWSWIRE
                    </div>
                    <div style={{ fontSize:9, color:"#334155", marginTop:3 }}>
                      ROUND-WISE DISRUPTIONS AND SILENT STORY BEATS
                    </div>
                  </div>
                  <div style={{ flex:1, overflowY:"auto", padding:"10px 12px" }}>
                    {newsRoundGroups.length === 0 ? (
                      <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
                        justifyContent:"center", height:"100%", gap:10 }}>
                        <div style={{ fontSize:28, opacity:0.15 }}>📰</div>
                        <div style={{ color:"#1e293b", fontSize:11, letterSpacing:"0.15em" }}>
                          AWAITING NEWS
                        </div>
                      </div>
                    ) : newsRoundGroups.slice(0, 3).map(([roundKey, items]) => (
                      <div key={roundKey} style={{ marginBottom:14 }}>
                        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                          <div style={{ fontSize:9, color:"#94a3b8", letterSpacing:"0.12em", fontWeight:800 }}>
                            {roundKey === "general" ? "GENERAL FEED" : `ROUND ${roundKey}`}
                          </div>
                          <div style={{ fontSize:8, color:"#334155" }}>{items.length} item{items.length === 1 ? "" : "s"}</div>
                        </div>
                        {items.slice(0, 4).map((item, index) => {
                          ...
                          const tone = item.sentiment === "bull"
                            ...
                            : item.sentiment === "bear"
                              ...
                              : "#38bdf8";
                          return (
                            <div key={`${roundKey}-${item.ticker}-${item.time || index}-${index}`} style={{
                              background:"#0a0f1e",
                              border:`1px solid ${tone}20`,
                              borderLeft:`3px solid ${tone}`,
                              borderRadius:8,
                              ...
                              <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4, flexWrap:"wrap" }}>
                                {!item.hideTicker && (
                                  <span style={{ color:stock.color, fontWeight:800, fontSize:11 }}>{item.ticker}</span>
                                )}
                                {item.tag && (
                              ...
                              <div style={{ fontSize:11, color:"#e2e8f0", lineHeight:1.45, fontWeight:600 }}>
                                {item.headline}
                              </div>
                              {item.detail && (
                              ...
                              {Array.isArray(item.playerClues) && item.playerClues.length > 0 && (
                                <div style={{ marginTop:7, display:"grid", gap:4 }}>
                                  {item.playerClues.slice(0, 3).map((clue, clueIndex) => (
                                    <div key={`${clueIndex}:${clue}`} style={{ display:"flex", gap:6, fontSize:9, color:"#94a3b8", lineHeight:1.45 }}>
                                      <span style={{ color:"#38bdf8", fontWeight:800, flexShrink:0 }}>{clueIndex + 1}.</span>
                                      <span>{clue}</span>
                                    </div>
                                  ))}
                              ...
                              {item.macroQuestion && (
                                <div style={{ marginTop:7, padding:"7px 8px", background:"#0f172a", border:"1px solid #1e293b", borderRadius:7 }}>
                                  <div style={{ fontSize:7, color:"#fbbf24", fontWeight:800, letterSpacing:"0.12em", marginBottom:4 }}>
                                    TEAM CHALLENGE
                                  </div>
                                  <div style={{ fontSize:9, color:"#e5e7eb", lineHeight:1.5 }}>
                                    {item.macroQuestion}
                                  </div>
                                </div>
                              )}
                ...
                <div style={{ minHeight:0, display:"flex", flexDirection:"column" }}>
                  <div style={{ padding:"10px 12px 8px", borderBottom:"1px solid #0f172a" }}>
                    <div style={{ fontSize:11, fontWeight:800, color:"#f1f5f9", letterSpacing:"0.12em" }}>
                      LIVE TRADES
                    </div>
                    <div style={{ fontSize:9, color:"#334155", marginTop:3 }}>REAL-TIME EXECUTION FLOW</div>
                  </div>
                  <div style={{ flex:1, overflowY:"auto", padding:"10px 12px" }}>
