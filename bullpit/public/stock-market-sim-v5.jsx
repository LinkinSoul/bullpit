...
const POWERUP_CARDS = [
  ...
  { id:"freeze",   icon:"🧊", name:"Freeze",    color:"#38bdf8", desc:"Trading locked for 90 seconds — prices keep moving", effect:{ type:"freeze", duration:90 } },
  { id:"wildcard", icon:"🎲", name:"Wild Card", color:"#e879f9", desc:"Redistributes 10% of cash randomly between teams",  effect:{ type:"shuffle", pct:10 } },
];
              ...
              <div style={{ borderTop:"1px solid #1e293b", padding:"14px",
                ...
                <div style={{ marginBottom:12 }}>
                  ...
                  {[
                    ...
                    { label:"Portfolio Beta", formula:"1 − |Holdings concentration|", value:e.beta?.toFixed(3), pts:e.t2c, max:10, rank:null, n, tip:"Low beta = independent thinking, not just indexing" },
                  ].map(ind => renderIndicator(ind, color))}
                </div>
                ...
                <div style={{ marginBottom:12 }}>
                  ...
                  {[
                    { label:"Win Rate", formula:"Profitable sells ÷ Total sells × 100", value:`${e.winRate}%`, pts:e.t3a, max:8, rank:rankWinRate[e.name], n, tip:"% of your closed trades that made money" },
...
function EmergencyModal({ events, bufferLeft, onApply, onClose, isFirstRound }) {
  ...
  }, [bufferLeft, isFirstRound, onApply, onClose]);
      ...
      <div style={{ width:"min(680px,95vw)", background:"#0a0f1e",
        ...
        <div style={{ padding:"14px 22px", borderTop:"1px solid #1e293b",
          ...
          <div style={{ fontSize:9, color:"#334155", letterSpacing:"0.1em" }}>
            {applied ? "✓ DISRUPTION APPLIED — LAUNCHING ROUND…" : "⏳ AUTO-STARTING WHEN COUNTDOWN REACHES 0"}
          </div>
...
// ─── STOCK DETAIL MODAL
function StockDetailModal({ stock, sector, price, prevPrice, history, holdings, cash, canTrade, onBuy, onSell, onClose, transactions, aiLog }) {
  ...
  const myBuys  = (transactions||[]).filter(t => t.type==="BUY"  && t.ticker===stock.ticker).length;
  const mySells = (transactions||[]).filter(t => t.type==="SELL" && t.ticker===stock.ticker).length;
              ...
              {held && (
                <div style={{ background:`${color}0a`, border:`1px solid ${color}25`,
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, fontSize:11 }}>
                    <div><div style={{ color:"#334155", fontSize:9, marginBottom:2 }}>SHARES</div>
                      <div style={{ color:"#f1f5f9", fontWeight:700 }}>{heldQty}</div></div>
                    ...
                    <div><div style={{ color:"#334155", fontSize:9, marginBottom:2 }}>VALUE</div>
        ...
        <div style={{ borderTop:`1px solid ${color}22`, background:"#040b17",
          ...
          {!canTrade && (
            <div style={{ fontSize:11, color:"#ef4444", marginBottom:8, textAlign:"center",
              ...
              ⚠ Trading locked — wait for a live round
            </div>
          )}
          <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:10, color:"#334155", marginTop:2 }}>
                ...
                {heldQty>0 && <span style={{ marginLeft:8,color:"#475569" }}>· Held: {heldQty}</span>}
              </div>
            </div>
...
export default function App() {
  ...
  const gameStartedRef  = useRef(false); // true once Round 1 has actually launched
  const currentBufferDurationRef = useRef(BUFFER_SECS); // adjustable: 90s normal, 30s political
  ...
  const saveScore = useCallback(async (name, color, total, cash_, holdings_, transactions_, initC, maxDD) => {
    ...
    try {
      await window.storage.set(`lb:${name}`, JSON.stringify({
        ...
        uniqueSectors, closedTrades: closedSells.length, wins: wins_,
  ...
  const pushGMState  = useCallback(async (o={}) => {
    try { await window.storage.set("gm:state", JSON.stringify({ phase:phaseRef.current, round:roundNum, roundLeft:timeLeft, bufferLeft, initCash, sentiment, volMultiplier, leaderHidden, frozenUntil, roundRulesIdx:roundNum-1, ...o }), true); } catch {}
  ...
  }, [gamePhase, disruptions, pushPrices]);
...
qty 1-15. Use REAL tickers from: ${tickerList.slice(0,200)}`,
  ...
  const genNews = useCallback(async (newP, prevP) => {
    ...
    try {
      ...
      if (h) setNews(p => [{ticker:top.ticker,sectorId:top.sectorId,headline:h,
        sentiment:top.chg>0?"bull":"bear",time:nowShort()},...p.slice(0,24)]);
    } catch {}
  }, []);
  ...
  const startNextRound = useCallback(() => {
    ...
    setPredictionOpen(false); setPredResult(null);
  ...
  const scheduleInRoundDisruptions = useCallback((roundIndex, roundDuration, roundStartTime) => {
    ...
    const fireAiDisruption = async (count = 1) => {
      for (let i = 0; i < count; i++) {
        ...
        setNews(prev => [...events.map(e=>({
          ticker:e.ticker, sectorId, headline:e.headline, sentiment:e.impact>0?"bull":"bear", time:nowShort()
        })), ...prev].slice(0,25));
      }
    };
  ...
  const gmCmd = useCallback((cmd, payload={}) => {
    if (cmd==="start")     { const st=Date.now();setStartTime(st);setGamePhase("running");setTimeLeft(roundDur);pushGMState({phase:"running",round:roundNum,roundLeft:roundDur}); }
      ...
      if (card.effect.type === "market") {
        ...
        pushGMState({ frozenUntil: until });
        sendBcast(`🧊 FREEZE CARD! Trading locked for ${card.effect.duration} seconds — prices keep moving!`);
        setTimeout(() => { setFrozenUntil(null); pushGMState({ frozenUntil:null }); sendBcast("🔓 Freeze lifted — trading resumes!"); }, card.effect.duration*1000);
        ...
        pushGMState({ wildcard: { pct:card.effect.pct, ts:Date.now() } });
      }
    }
  ...
  function execBuy(ticker, qty) {
    ...
    const newHolding = ex
      ? { qty:ex.qty+qty, avgCost:(ex.totalCost+total)/(ex.qty+qty), totalCost:ex.totalCost+total }
      : { qty, avgCost:p, totalCost:total };
    setCash(c => c - total);
    setHoldings(h => ({ ...h, [ticker]:newHolding }));
  ...
  function execSell(ticker, qty) {
    ...
    setHoldings(h => {
      ...
      else n[ticker] = { qty:newQty, avgCost:pos.avgCost, totalCost:pos.avgCost*newQty };
      return n;
    });
        ...
        <div style={{ overflowY:"auto", padding:"20px 24px" }}>
          ...
          <div style={{ background:"#060c18", border:"1px solid #1e293b", borderRadius:10,
            ...
            {[
              ...
              ["Power-Ups",    "GM has 4 one-time-use cards: Tsunami, Moon Shot, Freeze, Wild Card"],
            ].map(([k,v]) => (
          ...
          <div style={{ background:"#060c18", border:"1px solid #1e293b",
            ...
            <div style={{ marginBottom:10 }}>
              ...
              {[
                ...
                ["10pts", "Portfolio Beta",        "1 − |Holdings concentration ratio|",               "Low beta = independent thinking, not just buying the index."],
              ].map(([pts,name,formula,why]) => (
                <div key={name} style={{ display:"flex", gap:10, marginBottom:6, fontSize:10 }}>
            ...
            <div style={{ marginBottom:12 }}>
              ...
              {[
                ["8pts", "Win Rate",              "Profitable sells ÷ Total sells × 100",    "% of closed trades that made money."],
          ...
          <div style={{ background:"#060c18", border:"1px solid #38bdf844",
            ...
            {[
              ...
              "Have the Freeze card ready for moments when one team is running away with the lead",
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                ...
                <div style={{ background:"#0f172a", border:"1px solid #1e293b", borderRadius:8, padding:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                    ...
                    <div style={{ fontSize:8, color:"#334155" }}>{4 - usedCards.size} remaining</div>
                  </div>
                    ...
                    {POWERUP_CARDS.map(card => {
                      ...
                      return (
                        <button key={card.id} onClick={() => gmCmd("powerup",{id:card.id})}
                          ...
                          <div style={{ fontSize:14, marginBottom:2 }}>{card.icon}</div>
                          ...
                          {used && <div style={{ fontSize:8, color:"#ef4444", marginTop:2 }}>USED</div>}
                        </button>
                      );
                    })}
                  </div>
                  <button onClick={() => gmCmd("openPrediction")} disabled={predictionOpen || gamePhase!=="running"}
                    ...
                    {predictionOpen ? "🎯 Prediction Market OPEN" : "🎯 Open Prediction Market"}
                  </button>
                </div>
                ...
                <div style={{ background:"#0f172a", borderRadius:8, padding:10, fontSize:10, color:"#64748b", lineHeight:1.9 }}>
                  <div style={{ color:"#fbbf24", fontWeight:700, marginBottom:4 }}>5-PILLAR SCORING</div>
    ...
    <div style={{ minHeight:"100vh", background:"#020817", color:"#f1f5f9",
      ...
      {isFrozen && (
        <div style={{ position:"fixed", top:0, left:0, right:0, zIndex:8900,
          ...
          <div style={{ flex:1 }}>
            <div style={{ color:"#38bdf8", fontWeight:800, fontSize:13 }}>TRADING FROZEN</div>
            <div style={{ color:"#64748b", fontSize:10 }}>GM activated Freeze card — prices keep moving but you cannot trade</div>
          </div>
      ...
      {/* ── PREDICTION MARKET POPUP ── */}
      {predictionOpen && !playerPrediction && screen === "player" && (
        <div style={{ position:"fixed", inset:0, zIndex:8800, background:"rgba(2,8,23,0.93)",
          display:"flex", alignItems:"center", justifyContent:"center",
          fontFamily:"'JetBrains Mono',monospace" }}>
          <div style={{ width:"min(480px,95vw)", background:"#0a0f1e",
            border:"2px solid #00f5c4", borderRadius:14, padding:"24px", animation:"fadeup 0.3s" }}>
            {(() => {
              ...
              return (
                <>
                  <div style={{ textAlign:"center", marginBottom:16 }}>
                    ...
                    <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800,
                      fontSize:16, color:"#00f5c4", marginBottom:4 }}>PREDICTION MARKET</div>
                    <div style={{ fontSize:9, color:"#7c3aed", marginBottom:8,
                      padding:"2px 8px", borderRadius:4, background:"#7c3aed15",
                      display:"inline-block" }}>
                      CONCEPT: {pq.concept}
                    </div>
                    <div style={{ fontSize:12, color:"#94a3b8", lineHeight:1.6 }}>{pq.question}</div>
                    <div style={{ fontSize:9, color:"#334155", marginTop:6 }}>
                      Correct answer = +8% capital bonus at round end
                    </div>
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:12 }}>
                    {pq.options.map(opt => (
                      <button key={opt.id} onClick={() => { setPlayerPrediction(opt.id); setPredictionOpen(false); }}
                        style={{ padding:"10px 14px", background:"#0f172a",
                          border:"1px solid #1e293b", borderRadius:8,
                          color:"#94a3b8", cursor:"pointer", fontFamily:"inherit",
                          fontSize:11, textAlign:"left", lineHeight:1.5,
                          transition:"all 0.15s" }}>
                        <span style={{ color:"#00f5c4", fontWeight:700, marginRight:8 }}>
                          {opt.id.toUpperCase()}.
                        </span>
                        {opt.text}
                      </button>
      ...
      {gamePhase === "running" && (() => {
        ...
        return (
          <div style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:7000,
            ...
            {playerPrediction && <span style={{ fontSize:9, padding:"2px 6px",
              background:"#00f5c420", border:"1px solid #00f5c440", color:"#00f5c4",
              borderRadius:4 }}>Predicted: {SECTORS.find(s=>s.id===playerPrediction)?.icon} {playerPrediction}</span>}
          </div>
        );
      })()}
          ...
          <>
            ...
            {(()=>{
              ...
              const renderMover=(st,isGainer)=>{
                ...
                const qBuy=canTrade&&cash>=st.p, qSell=canTrade&&held>0;
                ...
                return (
                  <div key={s.ticker} style={{
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", marginTop:5, fontSize:9 }}>
                      ...
                      {held>0 && <span style={{ color:s.color }}>×{held}</span>}
                    </div>
            ...
            {selTicker && selStock && (
              <div style={{ background:"#0a0f1e", border:`1px solid ${selStock.color}55`,
                  ...
                  <div>
                    ...
                    {holdings[selTicker] && (
                      <div style={{ marginTop:6, fontSize:11, color:"#475569" }}>
                        Avg cost: {fmtUSD(holdings[selTicker].avgCost)} · Held: {holdings[selTicker].qty} shares
                        · Unrealized: {' '}
                        <span style={{ color:(prices[selTicker]-holdings[selTicker].avgCost)>=0?"#00f5c4":"#ef4444", fontWeight:700 }}>
                          {fmtUSD((prices[selTicker]-holdings[selTicker].avgCost)*holdings[selTicker].qty)}
                          {' '}({fmt(((prices[selTicker]-holdings[selTicker].avgCost)/holdings[selTicker].avgCost)*100)}%)
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div style={{ marginTop:8, fontSize:10, color:"#475569" }}>
                  ...
                  {!canTrade && <span style={{ color:"#ef4444", marginLeft:10 }}>⚠ Trading locked outside live rounds</span>}
                </div>
              </div>
            )}
          </>
              ...
              return (
                <div style={{ background:"linear-gradient(135deg,#0a0f1e,#060c18)",
                  ...
                  <div style={{ background:"#020817", borderRadius:10, padding:"12px 14px", marginBottom:8 }}>
                    ...
                    <Row label="Portfolio Beta" formula="1 − |mkt correlation|"         value={fmt(sc.beta, 2)}                  pts={sc.t2c} max={10} color="#ddd6fe" />
                  </div>
                            ...
                            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:8 }}>
                              <div>
                                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                                  ...
                                  <span style={{ fontSize:10, color:"#475569" }}>{pos.qty} shares</span>
                                </div>
                      ...
                      return (
                        <div key={tx.id||i} style={{
                          ...
                          <div style={{ color:"#64748b" }}>
                            {isSell ? fmtUSD(tx.avgCostAtSell||0) : fmtUSD(tx.avgCostAtBuy||tx.price)}
                          </div>
                          <div style={{ color:isSell?(tx.gain>=0?"#00f5c4":"#ef4444"):"#475569", fontWeight:700 }}>
                            {isSell ? (tx.gain>=0?"+":"")+fmtUSD(tx.gain||0) : "—"}
                          </div>
                          <div style={{ color:isSell?(tx.gainPct>=0?"#00f5c4":"#ef4444"):"#475569", fontWeight:700 }}>
                  ...
                  {analytics.totalRealized !== 0 && (
                    <div style={{ padding:"10px 14px", borderTop:"1px solid #111827",
                      ...
                      <span style={{ fontSize:10, color:"#475569" }}>
                        {analytics.wins}W / {analytics.losses}L on {transactions.filter(t=>t.type==="SELL").length} closed trades
                      </span>
