...
function makeDerivativeInstrument({ round, kind, stock, strike }) {
  const labelBase = kind === "future"
    ? `${stock.ticker} Round Future`
    : kind === "call"
      ? `${stock.ticker} Round Call`
      : `${stock.ticker} Round Put`;
  return {
    id: `${kind.toUpperCase()}:${stock.ticker}:R${round}`,
    round,
    kind,
...
function buildDerivativeCatalog({
  ...
  return getDerivativeUnderlyings().flatMap(stock => {
    const strike = roundBases?.[stock.ticker] || prices[stock.ticker] || stock.basePrice || 100;
    const instruments = [
      makeDerivativeInstrument({ round, kind:"future", stock, strike }),
      makeDerivativeInstrument({ round, kind:"call", stock, strike }),
      makeDerivativeInstrument({ round, kind:"put", stock, strike }),
    ];
...
function calcDerivativePositionValue(position, quote) {
  ...
  return quote.mark * qty * multiplier;
}
...
function calcDerivativeCloseValue(position, quote) {
  ...
  return quote.bid * qty * multiplier;
}
...
function calcDerivativeExposure(position, quote) {
  ...
  if (!qty) return 0;
  return (quote.riskEquivalent || 0) * qty;
}
...
function calcDerivativePortfolioMetrics(derivativePositions = {}, derivativeQuoteMap = {}) {
  ...
  entries.forEach(([id, position]) => {
    ...
    const unrealized = position.kind === "future"
      ? currentValue
      : ((quote.mark - (position.avgCost || 0)) * (position.qty || 0) * (position.multiplier || quote.multiplier || 1));
...
function inferTradeFlowDirection(trade = {}) {
  ...
  if (action === "SELL" || action === "SHORT" || action === "FUT SHORT" || action === "PUT BUY") return -1;
...
function App() {
  ...
  }, [roundNum, timeLeft, bufferLeft, bufferEndsAt, initCash, sentiment, volMultiplier, leaderHidden, frozenUntil, haltedUntil, predictionOpen, portfolioResetSignal]);
  ...
  const autoDeleveragePortfolio = useCallback(() => {
      ...
      ...Object.entries(nextDerivativePositions).map(([id, position]) => {
        ...
        return {
          ...
          unrealized: position.kind === "future"
            ...
            : ((quote.bid - (position.avgCost || 0)) * (position.qty || 0) * (position.multiplier || quote.multiplier || 1)),
        };
      }).filter(Boolean)
    ]
    ...
    for (const candidate of rankPositions()) {
      ...
      if (candidate.kind === "equity") {
        ...
        const realizedValue = position.kind === "future"
          ? entryValue + calcDerivativeCloseValue(position, quote)
          : closeUnitPrice * qty * multiplier;
        const gain = position.kind === "future"
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      ...
      try {
        ...
        if (gs) {
          ...
          if (parsed.phase === "idle" && nextRound === 1 && prevRound > 1) {
    ...
    return () => clearInterval(id);
  }, [applyPortfolioReset, clearScoreLedgerState, finalizeScoreLedgerRound, loadLB, roundNum]);
...
qty 1-15. Use REAL tickers from: ${tickerList.slice(0,200)}`,
  ...
  const applyStorylineEventPack = useCallback((events, { tagOverride = null, broadcastText = null } = {}) => {
    ...
    updateNewsFeed(prev => [
      ...events.map(evt => ({
        ticker: evt.ticker,
        sectorId: evt.sectorId,
        headline: `${evt.eventIcon ? `${evt.eventIcon} ` : ""}${evt.eventName} — ${evt.headline}`,
        detail: evt.detail,
        sentiment: evt.impact > 0 ? "bull" : "bear",
        tag: tagOverride || evt.eventTag || "STORYLINE UPDATE",
        time: nowShort()
      })),
      ...prev
    ]);
  ...
  const generateDisruption = useCallback(async () => {
    ...
    setBufferEndsAt(endsAt);
    setGamePhase("buffer");
    pushGMState({ phase:"buffer", bufferLeft:BUFFER_SECS, bufferEndsAt:endsAt });
    setShowEmergency(true);
  }, [buildStorylineEvents, getRoundStoryline, pushDisrupts, pushGMState, roundNum]);
  ...
  const generatePoliticalDisruption = useCallback(async (eventIdxOrEvent) => {
    ...
    setBufferStart(now); setBufferLeft(POLITICAL_BUFFER_SECS); setBufferEndsAt(endsAt);
    setGamePhase("buffer");
    pushGMState({ phase:"buffer", bufferLeft:POLITICAL_BUFFER_SECS, bufferEndsAt:endsAt });
    setShowEmergency(true);
  ...
  const marginGraceSecs = marginGraceEndsAtRef.current
    ...
    : 0;
  const derivativeOrderExposure = selectedDerivative && selectedDerivativeQuote
    ? (selectedDerivative.kind === "future"
        ? (selectedDerivativeQuote.riskEquivalent || 0) * derivativeQty
        : (selectedDerivativeQuote.ask || 0) * derivativeQty * (selectedDerivative.multiplier || selectedDerivativeQuote.multiplier || 1))
    : 0;
  const derivativeCanOpenLong = !!(canTrade && selectedDerivative && selectedDerivativeQuote && derivativeQty > 0 && derivativeOrderExposure <= buyingPower && (!selectedDerivativePosition || selectedDerivative.kind !== "future" || selectedDerivativePosition.side !== "short"));
  const derivativeCanOpenShort = !!(canTrade && selectedDerivative?.kind === "future" && selectedDerivativeQuote && derivativeQty > 0 && derivativeOrderExposure <= buyingPower && (!selectedDerivativePosition || selectedDerivativePosition.side !== "long"));
  ...
  function openDerivativePosition(instrument, qty, requestedSide = "long") {
    ...
    const side = isFuture ? requestedSide : "long";
    if (existing && existing.kind === "future" && existing.side !== side) return;
    const multiplier = instrument.multiplier || quote.multiplier || 1;
    const orderExposure = (quote.riskEquivalent || 0) * qty;
    if (orderExposure > analytics.buyingPower) return;

    const openUnitPrice = isFuture
      ? (side === "short" ? quote.bid : quote.ask)
      : quote.ask;
    const premiumCost = isFuture ? 0 : (openUnitPrice * qty * multiplier);
    const nextQty = (existing?.qty || 0) + qty;
    const weightedAvg = existing?.qty
      ? (((existing.avgCost || 0) * existing.qty) + (openUnitPrice * qty)) / nextQty
      : openUnitPrice;

    if (premiumCost > 0) setCash(value => value - premiumCost);
    ...
    pushTrade({
      ...
      action: isFuture ? (side === "short" ? "FUT SHORT" : "FUT LONG") : `${instrument.kind.toUpperCase()} BUY`,
      ...
      derivativeSide: side,
      multiplier,
      flowDirection: isFuture ? (side === "short" ? -1 : 1) : (instrument.kind === "put" ? -1 : 1),
      flowWeight: (quote.riskEquivalent || openUnitPrice * multiplier) * qty * (isFuture ? 1 : 0.7),
    });
  }

  function closeDerivativePosition(instrumentId, qty) {
    ...
    if (position.kind === "future") {
      ...
      closeUnitPrice = quote.bid;
      realizedValue = closeUnitPrice * closeQty * multiplier;
    ...
    pushTrade({
      ...
      multiplier,
      flowDirection: position.kind === "future"
        ? (position.side === "short" ? 1 : -1)
        : (position.kind === "put" ? 1 : -1),
      flowWeight: (quote.riskEquivalent || closeUnitPrice * multiplier) * closeQty * (position.kind === "future" ? 1 : 0.6),
    });
  }
  ...
  function settleExpiredDerivatives(settlementRound = activeLedgerRoundRef.current || roundNum) {
    ...
    ids.forEach((id, index) => {
      ...
      if (position.kind === "future") {
        ...
        realizedValue = settlementPrice * qty * multiplier;
        gain = realizedValue - entryValue;
        ...
        <div style={{ overflowY:"auto", padding:"20px 24px" }}>
          ...
          <div style={{ background:"#060c18", border:"1px solid #1e293b", borderRadius:10,
            ...
            {[
              ...
              ["Storyline",    "Every round opens with an America-focused macro bulletin, then two quieter mid-round beats flow silently into News"],
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#020817", color:"#f1f5f9",
      ...
      {(marginAlert || (analytics.grossExposure > 0 && marginStatus !== "safe")) && (
        <div style={{
          margin:"14px 18px 0",
          padding:"12px 14px",
          borderRadius:10,
          border:`1px solid ${marginStatus === "call" ? "#ef444455" : marginStatus === "watch" ? "#fbbf2444" : "#00f5c444"}`,
          background: marginStatus === "call"
            ? "rgba(127,29,29,0.28)"
            : marginStatus === "watch"
              ? "rgba(120,53,15,0.24)"
              : "rgba(2,44,34,0.28)"
          ...
          <div style={{ display:"flex", justifyContent:"space-between", gap:12, alignItems:"center", flexWrap:"wrap" }}>
            <div>
              <div style={{ fontSize:10, fontWeight:800, letterSpacing:"0.1em", color:marginStatus === "call" ? "#ef4444" : marginStatus === "watch" ? "#fbbf24" : "#00f5c4" }}>
                {marginStatus === "call" ? "MARGIN CALL ACTIVE" : marginStatus === "watch" ? "MARGIN WATCH" : "MARGIN STATUS"}
              </div>
              <div style={{ fontSize:11, color:"#cbd5e1", marginTop:4 }}>
                {marginAlert?.text || `Maintenance buffer is ${fmt(analytics.marginBufferPct, 1)}%. Keep net equity above maintenance margin.`}
              </div>
            </div>
          ...
          <div style={{ background:"#060c18", border:"1px solid #1e293b", borderRadius:14, padding:18, marginTop:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12, marginBottom:14, flexWrap:"wrap" }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:"#64748b", marginTop:4, lineHeight:1.6 }}>
                    Every listed Bull Pit stock now has a round-expiry future, call, and put.
                    Futures consume margin; options cap downside to the premium paid.
                  </div>
                </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1.55fr 0.95fr", gap:14 }}>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:8 }}>
                  {filteredDerivativeCatalog.map(instrument => {
                    ...
                    const flowDisplay = getOrderFlowDisplay(flow);
                    return (
                      <button key={instrument.id} onClick={() => setSelectedDerivativeId(instrument.id)} style={{
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", gap:8, marginBottom:6 }}>
                          <div>
                            ...
                            <div style={{ fontSize:9, color:"#64748b" }}>{instrument.kind === "future" ? "ROUND FUTURE" : instrument.kind === "call" ? "ROUND CALL" : "ROUND PUT"}</div>
                          </div>
                          {position ? (
                        ...
                        <div style={{ fontSize:10, color:"#94a3b8", lineHeight:1.6 }}>
                          ...
                          <div>Risk / contract {fmtUSD(quote?.riskEquivalent || 0)}</div>
                    ...
                    <>
                      <div style={{ display:"flex", justifyContent:"space-between", gap:10, marginBottom:10 }}>
                        ...
                        <div style={{ textAlign:"right" }}>
                          ...
                          <div style={{ fontSize:9, color:(selectedDerivativeFlow?.color || "#64748b"), marginTop:4, fontWeight:700 }}>
                            {selectedDerivativeFlow?.label || "BALANCED"}
                          </div>
                        </div>
                      </div>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"repeat(2,minmax(0,1fr))", gap:8, marginBottom:12 }}>
                        {[
                          ...
                          { l:selectedDerivative.kind === "future" ? "MARGIN RISK" : "PREMIUM RISK", v:fmtUSD(selectedDerivativeQuote.riskEquivalent || 0), c:"#fbbf24" },
                          { l:"STRIKE", v:selectedDerivative.kind === "future" ? "—" : fmtUSD(selectedDerivative.strike || 0), c:"#a78bfa" },
                          { l:"TIME VALUE", v:selectedDerivative.kind === "future" ? fmtUSD(selectedDerivativeQuote.timeValue || 0) : fmtUSD(selectedDerivativeQuote.timeValue || 0), c:"#22c55e" },
                          { l:"ORDER FLOW", v:selectedDerivativeFlow ? `${selectedDerivativeFlow.buyPct}% BUY` : "50% BUY", c:selectedDerivativeFlow?.color || "#64748b" },
                          { l:"FLOW BIAS", v:selectedDerivativeFlow?.label || "BALANCED", c:selectedDerivativeFlow?.color || "#64748b" },
                        ].map(card => (
                      ...
                      {selectedDerivativePosition ? (
                        <div style={{ marginBottom:12, padding:"10px 12px", borderRadius:8, background:"#020817", border:"1px solid #1e293b" }}>
                          ...
                          <div style={{ fontSize:10, color:"#94a3b8", lineHeight:1.7 }}>
                            <div>{selectedDerivativePosition.kind === "future" ? `${selectedDerivativePosition.side.toUpperCase()} FUTURE` : selectedDerivative.kind.toUpperCase()} · {selectedDerivativePosition.qty} contract(s)</div>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                        ...
                        <div style={{ fontSize:11, color:"#64748b", lineHeight:1.7 }}>
                          <div>Order risk: <span style={{ color:"#f1f5f9", fontWeight:700 }}>{fmtUSD(derivativeOrderExposure)}</span></div>
                        ...
                        {selectedDerivative.kind === "future" ? (
                          <>
                            <button onClick={() => openDerivativePosition(selectedDerivative, derivativeQty, "long")} disabled={!derivativeCanOpenLong} style={{
                              ...
                              background:derivativeCanOpenLong ? "#00f5c4" : "#1e293b",
                              color:derivativeCanOpenLong ? "#04130a" : "#334155", fontWeight:800, cursor:derivativeCanOpenLong ? "pointer" : "not-allowed", fontFamily:"inherit"
                            }}>LONG FUTURE</button>
                          ...
                          <button onClick={() => openDerivativePosition(selectedDerivative, derivativeQty, "long")} disabled={!derivativeCanOpenLong} style={{
                            flex:1, minWidth:160, padding:"10px 0", border:"none", borderRadius:7,
                            background:derivativeCanOpenLong ? (selectedDerivative.kind === "call" ? "#38bdf8" : "#a78bfa") : "#1e293b",
                          ...
                          }}>{selectedDerivative.kind === "call" ? "BUY CALL" : "BUY PUT"}</button>
                        )}
                ...
                Object.entries(analytics.derivativeOpenPnL).map(([instrumentId, pos]) => {
                  ...
                  return (
                    <div key={instrumentId} style={{ background:"#0a0f1e", border:"1px solid #111827", borderRadius:10, padding:"12px 14px", marginBottom:6 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", gap:12, marginBottom:8 }}>
                        <div>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            ...
                            <span style={{ fontSize:9, color:"#475569" }}>{pos.kind === "future" ? `${pos.side.toUpperCase()} FUTURE` : pos.kind.toUpperCase()}</span>
                          </div>
                      ...
                      <div style={{ display:"flex", gap:6 }}>
                        <button onClick={() => { setDerivativeSearch(""); setDerivativeSector("all"); setDerivativeKind("all"); setSelectedDerivativeId(instrumentId); setTab("derivatives"); }}
                    ...
                    {derivativeTransactions.map((tx, index) => {
                      ...
                      return (
                        <div key={tx.id || index} style={{ display:"grid", gridTemplateColumns:"80px 90px 110px 80px 80px 90px 80px 90px", gap:0, padding:"10px 14px", borderBottom:"1px solid #060c18", background:index % 2 === 0 ? "#0a0f1e" : "#080d18", fontSize:11, alignItems:"center" }}>
                          <div style={{ color:"#475569", fontSize:9 }}>{tx.time}</div>
                          <div style={{ color:isClose ? "#ef4444" : "#38bdf8", fontWeight:700 }}>{tx.type.replace("DERIV_", "")}</div>
                          <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
